...
for(hierarchy_name in hierarchy_names){
  ...
  #id_learn <- c(validation$id, head(test$id, 48 * 30))
  #id_test <- tail(test$id, 48 * 62)
  
  id_learn <- head(test$id, 48 * 30 * 2)
  id_test <- tail(test$id, 48 * 32)
  ###  
  ...
  file_bf <- file.path(bf.folder, 
                       paste("bf_", hierarchy_name, ".Rdata", sep = "")) 
