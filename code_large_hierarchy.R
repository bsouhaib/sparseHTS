set.seed(1986)
################ Data Generation #####################
require(stats) # For generating data from ARIMA
require(mvtnorm) # To generate errors for ARIMA process
require(hts)  # For hierarchical forecasting
require(magic)
require(matrixcalc)   # To check the positive definitness of the sample covariance matrix
require(Matrix)


# Generating AR, MA terms with the correspoding coefficients - For correlated series
...
res <- lapply( seq(ncol(ally)), function(j){Arima(ally[, j])$coef })
myS <- smatrix(sim.hts)
